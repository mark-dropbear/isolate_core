...
class PersonController {
  ...
  PersonController(this._storage);

  Future<TransportResponse> handleList() async {
    try {
      final dataset = await _storage.queryResources(type: Vocab.personClass);
  ...
  Future<TransportResponse> handleCreate(TransportRequest request) async {
    try {
      ...
      final newName = ResourceName.generate('persons').toString();
      ...
      await LinkSyncUtils.syncBidirectionalLinks(
        ...
        newDataset: created,
        forwardPredicate: Vocab.worksFor,
        reversePredicate: Vocab.employee,
      );
